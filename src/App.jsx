...
function App() {
  ...
  const worksRef = useRef(
    DefaultWorksList.sort(SORT_FN_MAP[currSortRef.current]),
  );
  const [dynamicWorksList, setList] = useState(
    Array.from({ length: worksRef.current.length }, () => DefaultItem),
  );
  ...
  useEffect(() => {
    worksRef.current = DefaultWorksList.filter(
      (w) => filterState === "none" || w.filter === filterState,
    );
    setList(Array.from({ length: worksRef.current.length }, () => DefaultItem));
  ...
  const createFilterObj = (titles) => {
    return Array.from(titles, (title) => {
      return {
        title,
        key: title.slice(0, 3),
        handler: () => handleFilter(title),
        active: filterState === title,
      };
    });
  };
  ...
  const createSortObj = (titles) => {
    return Array.from(titles, (title) => {
      return {
        title,
        key: title.slice(0, 3),
        handler: () => handleSort(title),
        active: currSortRef.current === title,
      };
    });
  };
        ...
        <div className="flex flex-col gap-16 p-4 text-xs sm:text-base h-full">
          ...
          <div className="flex w-full justify-between items-end -mb-8">
            ...
            <div className="flex gap-2">
              <Dropdown
                text={"filter"}
                options={createFilterObj(["freelance", "commercial"])}
              />
              <Dropdown
                text={"sort"}
                options={createSortObj(["new-old", "old-new", "a-z", "z-a"])}
              />
            </div>
          </div>
